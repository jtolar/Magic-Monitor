...
namespace MagicMonitor.Common.SoftWAP
{
    public class SoftWAPService
    {
        ...
        private static int SetupPin = 5;

        public static void StartSoftWap(SoftApConfiguration configuration, int setupPin)
        {
            ...
            {
                WirelessAP.SetWifiAp(configuration);
                _wifiApMode = true;
            }
            else
            {
                _wifiApMode = Wireless80211.ConnectOrSetAp(configuration);
            }

            Console.WriteLine(
                $"Connected with wifi credentials. IP Address: {(_wifiApMode ? WirelessAP.GetIP() : Wireless80211.GetCurrentIPAddress())}");
